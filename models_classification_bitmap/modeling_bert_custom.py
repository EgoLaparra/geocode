...
class BertForSequenceClassification(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.classifier_1 = nn.Linear(config.hidden_size + 2*self.num_labels, config.hidden_size + 2*self.num_labels)
        self.classifier_2 = nn.Linear(config.hidden_size + 2*self.num_labels, config.hidden_size + 2*self.num_labels)
        self.classifier_final = nn.Linear(config.hidden_size + 2*self.num_labels, self.num_labels)
        self.init_weights()

    def forward(self, input_ids, attention_mask=None, token_type_ids=None,
                position_ids=None, head_mask=None, para_links=None, labels=None):
        ...
        #out_para_links = out_para_links.view(-1, out_para_links.size(1))
        # out_para_links = self.conv(para_links)
        # out_para_links = out_para_links.permute(0,2,1)
        ...
        if labels is not None:
            if self.num_labels == 1:
                ...
                loss = loss_fct(logits.view(-1, self.num_labels), labels.view(batch_size,-1))
...
class BertForMultipleChoice(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.bert = BertModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.dense = nn.Linear(config.hidden_size+1+681, config.hidden_size+1+681)
        self.classifier = nn.Linear(config.hidden_size+1+681, 150)
        self.classifer_final = nn.Linear(150, 1)
        self.init_weights()

    def forward(self, input_ids, attention_mask=None, token_type_ids=None,
                position_ids=None, head_mask=None, labels=None, populations=None, feacodes=None):
        ...
        #populations_max = torch.max(populations, 1)[0]
        #log_populations = torch.div(populations, populations_max)
        #populations = populations.squeeze(0)
        ...
        x = self.dropout(final_pooled_output)
        x = self.dense(x)
        x = torch.relu(x)
        x = self.dropout(x)
        logits_temp = self.classifier(x)
        logits_temp = torch.tanh(logits_temp)
        logits_temp = self.dropout(logits_temp)
