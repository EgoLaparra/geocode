...
class BertForSequenceClassification(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.classifier_1 = nn.Linear(config.hidden_size + 4* 3 * 3 * 2, config.hidden_size + 4 * 3 * 3 * 2)
        self.classifier_2 = nn.Linear(config.hidden_size + 4 * 3 * 3 * 2, config.hidden_size + 4 * 3 * 3 * 2)
        self.classifier_final = nn.Linear(config.hidden_size + 4 * 3 * 3 * 2, self.num_labels)
        self.init_weights()
    ...
    def forward(self, input_ids, attention_mask=None, token_type_ids=None,
        ...
        para_links = para_links.view(-1, links_num, 3 * 3 * 2)
        ...
        final_pooled_output = torch.cat([pooled_output, out_para_links], dim=1)
