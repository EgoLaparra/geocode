 ...
 */
class SynchronousParser(grammar: SynchronousGrammar, subOptimal:Boolean = false) {
  ...
  def parseAll(sourceTokens: IndexedSeq[String]): IndexedSeq[Tree.NonTerminal] = {
    ...
    val completes = if (!this.subOptimal)
      chart(sourceTokens.size)(0).completes
    else {
      val filered_chart = chart.filter(_.map(_ != null).exists(_ == true)).filter(_ (0).completes.nonEmpty)
      filered_chart.lastOption match {
        case Some(c) => c(0).completes
        case None => Set.empty
      }
    }
