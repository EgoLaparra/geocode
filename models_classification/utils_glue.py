...
class GeoComposeProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        ...
        return self._create_examples(self.pickle_load_large_file(os.path.join(data_dir, "model_input_desc_classification_relative_boundary_10_LOCATION_train.pkl")),
                                     self.pickle_load_large_file(os.path.join(data_dir, "model_input_paras_classification_relative_boundary_10_LOCATION_train.pkl")),
                                     self.pickle_load_large_file(os.path.join(data_dir, "model_input_target_classification_relative_boundary_10_LOCATION_train.pkl")),"train")

    def get_dev_examples(self, data_dir):
        """See base class."""
        logger.info("LOOKING AT {} dev".format(data_dir))
        return self._create_examples(self.pickle_load_large_file(os.path.join(data_dir, "model_input_desc_classification_relative_boundary_10_LOCATION_dev.pkl")),
                                     self.pickle_load_large_file(os.path.join(data_dir, "model_input_paras_classification_relative_boundary_10_LOCATION_dev.pkl")),
                                     self.pickle_load_large_file(os.path.join(data_dir, "model_input_target_classification_relative_boundary_10_LOCATION_dev.pkl")), "dev")
        ...
        for entity_id in desciptions.keys():
            ...
            for single_para in para.keys():
                for link_id in para[single_para].keys():
                    normalied_entity = np.array(para[single_para][link_id]) / norm_bench
                    para_entities.append(normalied_entity.tolist())
