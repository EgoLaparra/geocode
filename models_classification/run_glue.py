...
#from modeling_bert_custom import BertForMultipleChoice
from modeling_roberta_custom import RobertaForSequenceClassification
from modeling_bert_custom import BertForSequenceClassification
from utils_glue import convert_examples_to_features, processors
...
# ALL_MODELS = sum(
#     (tuple(conf.pretrained_config_archive_map.keys()) for conf in (BertConfig, XLNetConfig, RobertaConfig, XLMConfig, XLMRobertaConfig)), ()
# )
...
def train(args, train_dataset, model, tokenizer):
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    avg_loss.append((tr_loss - logging_loss) / args.logging_steps)
                    avg_loss_steps.append(global_step)
    ...
    if args.local_rank in [-1, 0]:
        tb_writer.close()
    loss_dit= {'loss': avg_loss, 'steps':avg_loss_steps}

    return global_step, tr_loss / global_step, best_steps, loss_dit
...
def evaluate(args, model, tokenizer, prefix="", test=False):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        preds_temp = np.resize(preds, (preds.shape[0], 3, 3, 2))
        ...
        with open(output_eval_file, "w") as writer:
            ...
            for key in sorted(result.keys()):
                ...
                writer.write("%s = %s\n" % (key, str(result[key])))
    return results, preds_score_single
...
def main():
    ...
    parser.add_argument("--n_labels", default=None, type=int, required=True,
                        help="The length of candidate list.")
    ...
    if args.do_train:
        ...
        global_step, tr_loss, best_steps, loss_dic = train(args, train_dataset, model, tokenizer)
        train_loss_file = os.path.join(args.output_dir, "train_loss.json")
        with open(train_loss_file, "w") as writer:
            json.dump(loss_dic, writer)
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model.to(args.device)
            result, preds_score = evaluate(args, model, tokenizer, prefix=prefix)
            result = dict((k + "_{}".format(global_step), v) for k, v in result.items())
            #preds = dict((k + "_{}".format(global_step), v) for k, v in preds.items())
            preds_score = dict((k + "_{}".format(global_step), v) for k, v in preds_score.items())
            results.update(result)
            #preds_all.update(preds)
            preds_score_all.update(preds_score)
